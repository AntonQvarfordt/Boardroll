...
namespace CodeStage.Maintainer.Core
{
	...
	public class AssetsMap
	{
		...
		private static bool UpdateMap(AssetsMap map)
		{
			...
			if (EditorUtility.DisplayCancelableProgressBar("Updating Assets Map, phase 1 of 4", "Checking existing assets in map...", 0))
			{
			...
			for (var i = count - 1; i > -1; i--)
			{
				...
				{
					var index = count - i;
					if (EditorUtility.DisplayCancelableProgressBar("Updating Assets Map, phase 1 of 3", "Checking existing assets in map..." + index + "/" + count, (float) index / count))
					{
						EditorUtility.ClearProgressBar();
						Debug.Log(Maintainer.ConstructLog("Assets Map update was canceled by user."));
						return false;
			...
			// ----------------------------------------
			if (EditorUtility.DisplayCancelableProgressBar("Updating Assets Map, phase 2 of 3", "Looking for new assets...", 0))
			{
			...
			for (var i = 0; i < count; i++)
			{
...
#endif
				{
					...
					if (EditorUtility.DisplayCancelableProgressBar("Updating Assets Map, phase 2 of 3",
			...
			if (EditorUtility.DisplayCancelableProgressBar("Updating Assets Map, phase 3 of 3", "Generating links...", 0))
			{
				Debug.Log(Maintainer.ConstructLog("Assets Map update was canceled by user."));
				return false;
			}
...
#endif
				{
					if (EditorUtility.DisplayCancelableProgressBar("Updating Assets Map, phase 3 of 3", "Generating links..." + (i + 1) + "/" + count, (float)i / count))
					{
						Debug.Log(Maintainer.ConstructLog("Assets Map update was canceled by user."));
						return false;
					}
