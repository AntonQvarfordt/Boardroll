...
namespace RhythmTool
{
    ...
    public class Chromagram : Analysis<Chroma>
    {
        ...
        public override void Initialize(int sampleRate, int frameSize, int hopSize)
        {
            ...
            endNote = Mathf.Min(endNote, FrequencyToNote(sampleRate / downsampleFactor));
            ...
            for (int i = 0; i < noteIndices.Length; i++)
            {
                ...
                noteIndices[i] = FrequencyToIndex(frequency, bufferSize / 2, sampleRate / downsampleFactor) - 1;
            }
        ...
        private static int FrequencyToNote(float frequency)
        {
            return Mathf.RoundToInt(69 + 12 * Mathf.Log(frequency / 440, 2));
        }
